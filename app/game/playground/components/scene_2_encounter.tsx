...
interface Scene2EncounterProps {
  variantId?: number;
}

export default function Scene2Encounter({ variantId = 1 }: Scene2EncounterProps) {
  ...
  return (
    <svg viewBox="0 0 200 120" className="w-full h-full">
      ...
      {/* 1. The Penguin (rotated 90 degrees to stand up, positioned on the left) */}
      <g id="pinguino-contenedor" transform="translate(99.601913, 50.428756)">
        <g transform="translate(-47.91, 37.303)">
          <motion.g
            id="pinguino-rotacion-entrada"
            style={{ transformOrigin: "0px 0px" }}
            // Plays the stand-up animation from 0 to 90 degrees
            initial={{ rotate: 0 }}
            animate={{ rotate: 90 }}
      ...
      {/* 2. The Subconscious Eye (on the right, floating) */}
      <g transform={transformAttr}>
        <motion.g
          id="ojo-subconsciente-entrada"
          // Slow fade-in and scale expansion after the penguin stands up
          initial={{ opacity: 0, scale: 0.6, y: -15 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          transition={{ duration: 2.8, ease: "easeOut", delay: 1.8 }}
        >
          {variantId === 1 && (
            <motion.g
