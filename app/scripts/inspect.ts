...
async function inspectDatabase() {
    ...
    try {
        ...
        console.log(`\n[players] Filas: ${players.rowCount}`);
        if (players.rowCount > 0) console.table(players.rows);

        // 2. Interaction Logs
        const logs = await client.sql`SELECT * FROM interaction_logs LIMIT 50;`;
        console.log(`\n[interaction_logs] Filas: ${logs.rowCount}`);
        if (logs.rowCount > 0) console.table(logs.rows);

        // 3. Likert Responses
        const survey = await client.sql`SELECT * FROM likert_responses LIMIT 50;`;
        console.log(`\n[likert_responses] Filas: ${survey.rowCount}`);
        if (survey.rowCount > 0) console.table(survey.rows);

        // 4. Marking Results
        const marking = await client.sql`SELECT * FROM marking_results LIMIT 50;`;
        console.log(`\n[marking_results] Filas: ${marking.rowCount}`);
        if (marking.rowCount > 0) console.table(marking.rows);

        // 5. Narrative State
        const narrative = await client.sql`SELECT * FROM narrative_state LIMIT 50;`;
        console.log(`\n[narrative_state] Filas: ${narrative.rowCount}`);
        if (narrative.rowCount > 0) console.table(narrative.rows);
